...
function handleLogin(event) {
  ...
  if (!users[email] || users[email].password !== password) {
    authMessage.textContent = "Ungültige E-Mail oder Passwort.";
    return;
  }
  saveSession({ email });
  showApp(email);
...
function handleRegister(event) {
  ...
  if (password.length < 5) {
    authMessage.textContent = "Passwort muss mindestens 5 Zeichen lang sein.";
    return;
  }
  if (password !== confirm) {
...
function renderContactList() {
  ...
  if (contacts.length === 0) {
    ...
    empty.textContent = "Noch keine Kontakte. Füge einen neuen Kontakt hinzu.";
...
function updateChatHeader() {
  ...
  if (contact) {
    ...
    chatSubtitle.textContent = `Privater Chat mit ${contact.name}`;
    chatInput.disabled = false;
  } else {
    chatTitle.textContent = "Kein Kontakt ausgewählt";
    chatSubtitle.textContent = "Wähle einen Kontakt aus der Liste oder füge einen neuen hinzu.";
...
function renderHireList() {
  ...
  if (hireRequests.length === 0) {
    const empty = document.createElement("div");
    empty.className = "empty-state";
    empty.textContent = "Noch keine erstellten Hire-Anfragen.";
...
function handleHireSubmit(event) {
  ...
  if (!name) {
    hireStatus.textContent = "Bitte gib einen Namen ein.";
    return;
  }
  if (!info) {
    hireStatus.textContent = "Bitte gib eine Info oder Nachricht ein.";
    return;
  }
  ...
  const subject = encodeURIComponent(`Neue Hire-Anfrage: ${item.title}`);
  const body = encodeURIComponent(`Neue Hire-Anfrage\n\nName: ${name}\nTelefon: ${phoneLabel}\nInfo: ${info}\nProjekt: ${item.title}\n\nBitte akzeptieren oder ablehnen:\n- Accept\n- Reject\n`);
  const mailtoURL = `mailto:${adminEmail}?subject=${subject}&body=${body}`;

  hireStatus.textContent = "E-Mail vorbereitet. Dein E-Mail-Programm öffnet sich gleich.";
...
function renderVideoList() {
  ...
  if (!loadedVideos.length) {
    const empty = document.createElement("div");
    empty.className = "empty-state";
    empty.textContent = "Noch keine Videos. Ziehe hier deine Arbeitsvideos hinein.";
